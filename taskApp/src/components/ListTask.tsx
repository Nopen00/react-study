...
const ItemTask = ({ task,handleUpdateTask, handleRemoveTask }: TaskItemProps) => {
  //Edit 클릭시
  ...
  // save 버튼 클릭시
  // 현재 task 의 text 를 변경한다 (원본복사, texta 만 변경)=> Main에서 내려온 함수 호출 => isEditing fales 로 변경
  const taskTextChage = () =>{
    handleUpdateTask({
      ...task,
      text:text,
  ...
  return (
    <div className="flex items-center justify-between px-3 py-2">
      ...
      <div className="flex items-center gap-2">
        {isEditing ? (
          <button onClick={taskTextChage} type="button" className="rounded border px-3 py-2 text-sm text-green-600 hover:text-green-800">
            Save
          </button>
        ) : (
          <button type="button" onClick={() =>setIsEditing(true)} className="rounded border px-3 py-2 text-sm text-green-600 hover:text-green-800">
            Edit
          </button>
        )}
...
const ListTask = ({ tasks, handleUpdateTask,handleRemoveTask }: TaskListProps) => {
  return (
